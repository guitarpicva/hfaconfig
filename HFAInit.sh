...
then
	...
	sudo apt install git dialog cmake libasound2-dev libudev-dev libtool dos2unix telnet mosquitto mosquitto-clients screen chrony gpsd libgps-dev -y
	# gpsd-clients package not really needed and avoids a lot of UI stuff and python3 which are large
	cd ~
